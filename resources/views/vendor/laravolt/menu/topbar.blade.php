        ...
        @auth
            <div class="item">
             ...
             @if(data_get($roleuser,'role_id')==1)
                    @if($pendding==0)
                     <div class="ui simple dropdown basic button top right pointing b-0 p-x-volt-0" style="padding: 3px 12px;">
                        <i class="big bell outline icon"></i><span id="notis"><b>{{$pendding}}</b></span>
                      
                    </div>
                    @else
                     <div class="ui simple dropdown basic button top right pointing b-0 p-x-volt-0" style="padding: 3px 12px;">
                        <i class="big bell outline icon" style="color: #fecb3a;"></i><span id="notis"><font color="red"><b>{{$pendding}}</b></font></span>
                        <i class="dropdown icon m-l-0 {{ config('laravolt.ui.color') }}"></i>
                        <div class="menu">
                            <div class="header"><span class="ui text {{ config('laravolt.ui.color') }}">Kelulusan Pengguna</span></div>

                            <div class="divider"></div>

                            <a href="{{ route('site::users.approveindex') }}" class="item"><b>Perlu Kelulusan Pengguna </b><div class="ui primary inverted basic label">{{$pendding}}</div></a>
