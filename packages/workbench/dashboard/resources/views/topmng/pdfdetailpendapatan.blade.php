...
<tr>
     <td><b>DAERAH</b></td><td width="2%">:</td><td  colspan="4">{{$daerah}}</td>
      @if($type!=0)
     <td><b>PENDAPATAN</b></td><td width="2%">:</td><td    colspan="4">{{$pendapatan}}</td>
     @endif
</tr>
  </table>
  <br>
...
<table width="100%" class="tabledata"  cellpadding="2" cellspacing="0">
    <thead style="background-color: lightgray;" style="page-break-inside: avoid;">
      ...
      <th width="5%" align="center"><b>BIL</b></th>
                  ...
                  <th  width="20%" align="center"><b>PEKERJAAN</b></th>
                   @if($type==0)
                  <th width="20%" align="center">PENDAPATAN</th>
                  @endif
        ...
        <tbody>
            <?php $i=1;?>
                @forelse($detailpendapatan as $key =>$value)
                  <tr style="vertical-align:top;">
                        <td>{{$i}}</a></td>
                         ...
                         @endif
                         <td>{{data_get($value,'parlimen.NamaParlimen')}}</a></td>
                         <td>{{data_get($value,'dun.NamaDun')}}</a></td>
                         <td>{{data_get($value,'daerah.NamaDaerah')}}</a></td>
                         <td>{{data_get($value,'mukim.NamaMukim')}}</a></td>
