<div class="ui buttons right floated">
 <a href="{!! URL::to('dashboard/exportjenisrumah/1/'.data_get($daerah,'id').'/'.$type) !!}" class="ui red button" >Muat Turun</a>
</div>
   <h4>DAERAH: {{data_get($daerah,'NamaDaerah')}}</h4>
   @if($type!=0)
   <h4>JENIS RUMAH: {{data_get($status,'description')}}</h4>
   @endif
          <table class="ui unstackable celled striped table" style="width:100%; font-size: 12px; padding-top: 10px" id="listjenisrumah">
                        <thead>
                            <tr>
                      ...
                      <?php $i=1; ?>
                     @forelse($detailjenisrumah as $key =>$data)
                     <tr  style="text-align: center;">
                         ...
                         @if($type==0)
                         <td>{{data_get($data,'jenis_rumah')}}</a></td>
                         @endif
