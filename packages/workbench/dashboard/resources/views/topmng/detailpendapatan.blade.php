<div class="ui buttons right floated">
 <a href="{!! URL::to('dashboard/exportdetailpendapatan/1/'.data_get($daerah,'id').'/'.$type) !!}" class="ui red button" >Muat Turun</a>
</div>
   <h4>DAERAH: {{data_get($daerah,'NamaDaerah')}}</h4>
   @if($type!=0)
   <h4>PENDAPATAN: {{$pendapatan}}</h4>
   @endif
          <table class="ui unstackable celled striped table" style="width:100%; font-size: 12px; padding-top: 10px" id="listincome">
                        <thead>
                            <tr>
                      ...
                      <?php $i=1; ?>
                     @forelse($detailpendapatan as $key =>$data)
                     <tr  style="text-align: center;">
                         ...
                         @endif
                         <td>{{data_get($data,'parlimen.NamaParlimen')}}</a></td>
                         <td>{{data_get($data,'dun.NamaDun')}}</a></td>
                         <td>{{data_get($data,'daerah.NamaDaerah')}}</a></td>
                         <td>{{data_get($data,'mukim.NamaMukim')}}</a></td>
