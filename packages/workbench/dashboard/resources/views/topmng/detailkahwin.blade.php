<div class="ui buttons right floated">
 <a href="{!! URL::to('dashboard/exportdetailkahwin/1/'.data_get($daerah,'id').'/'.$type) !!}" class="ui red button" >Muat Turun</a>
</div>
   <h4>DAERAH: {{data_get($daerah,'NamaDaerah')}}</h4>
   @if($type!=0)
   <h4>TARAF PERKAHWINAN: {{data_get($status,'description')}}</h4>
   @endif
          <table class="ui unstackable celled striped table" style="width:100%; font-size: 12px; padding-top: 10px" id="listkahwin">
                        <thead>
                            <tr>
                      ...
                      <?php $i=1; ?>
                     @forelse($detailkahwin as $key =>$data)
                     <tr  style="text-align: center;">
                         ...
                         @if($type==0)
                         <td>{{data_get($data,'taraf_kahwin')}}</a></td>
                         @endif
