...
<div id="actionbar" class="ui two column grid content__body p-3" >
    <div class="column middle aligned">
                <h3 class="ui header m-t-xs titleKetuaKampung ">
          DASHBOARD KETUA KAMPUNG- {{data_get($kampung,'NamaKampung')}}
        </h3>
    </div> 
            ...
            <form class="ui form">
                 <div class="two fields">
                  @if(data_get($roleuser,'role_id')==2)
                     ...
                     @elseif(data_get($roleuser,'role_id')==3 || data_get($roleuser,'role_id')==10)
                      <div class="field">
                          <label>Daerah</label>
                             <input type="text" name="daerah" id="daerah" readonly="readonly" value="{{data_get($daerah,'NamaDaerah')}}">
                     </div>
...
<script type="text/javascript">
  ...
  $(document).ready(function() 
  {
    ...
    if(mukimuser==''){
     ...
     if(kampunguser==''){
      var valkampunguser=0;
    }else{
      var valkampunguser=kampunguser;
    }
...
//alert ('here-->'+kampung);
 ...
 var kampunguser="{{$kampunguser}}";
 if(kampunguser!='')
 valkampung=kampunguser;

      $.ajax({ 
            type: "GET", 
             url: "{{ URL::to('/dashboard/countpetempatan/')}}?parlimen="+valparlimen+"&dun="+valdun+"&daerah="+valdaerah+"&mukim="+valmukim+"&catpetempatan="+valcat_petempatan+"&kampung="+valkampung+"&statKetuaKampung=1",
...
@if(!empty(Session::get('popupKampung')))
 
  
   //alert('test{{Session::get('popupKampung')}}');
//window.open("/info/{{Session::get('popupKampung')}}");
//let newTab = window.open();
 window.location.href = "{{env('BASEFOLDER')}}/info/{{Session::get('popupKampung')}}";
/*$.ajax({ 
            type: "GET", 
            url: "{{env('BASEFOLDER')}}/info/{{Session::get('popupKampung')}}",
            datatype : 'json',
...
@endif
 ...
 <div id="FormModal" class="modal2">

  <!-- Modal content -->
  <div class="modal-content2">
   <div align="right" ><span class="close2" onClick="document.getElementById('FormModal').style.display = 'none';lib.showLoading(); lib.doDim();location.reload();">&times;</span> </div>
   
<div id="contentFormModal">
</div>
