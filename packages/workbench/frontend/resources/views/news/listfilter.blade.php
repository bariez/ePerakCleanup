                            	...
                            	<div class="list-recent-jobs">
                                    <div class="card-job wow animate__animated animate__fadeIn">
                                        <div class="card-job-top">
											...
											{!! form()->open()->post()->action(route('frontend.postNewsFilter'))->attribute('id', 'formnewssearchlist')->multipart()->horizontal() !!}

												<div class="input-group">
													<input type="text" class="form-control hover-up" placeholder="Carian..." name="searchdata" value="{{ $request->data }}">
													<!-- <input type="text" class="form-control"> -->
													
													<select class="form-control hover-up" id="tahun" style="max-width: 30%" name="searchtahun" >  <!-- select-active -->
														<option value="">Tahun</option>
														
														@if(data_get($request, 'tahun'))
															<option value="{{ $request->tahun }}" selected="true">{{ $request->tahun }}</option>
														@else
														@endif

														@foreach($year->where('tarikh_notis_date', '!=', $request->tahun) as $key => $years)
