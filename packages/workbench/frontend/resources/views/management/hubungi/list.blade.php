...
<!-- <style>
	...
	<h4 class="ui top attached header">
		Senarai Hubungi Kami
	</h4>

	<div class="ui attached segment">
		<table id="listhubungi" class="ui celled table" style="width:100%">
			<thead>
				<tr>
					...
					<th style="text-align: center;">No Faksimili</th>
			...
			<tbody>
				...
				@forelse($data as $key => $value)
					<tr  style="text-align: center;">
						...
						<td style="text-align: left;">{{ data_get($value, 'alamat') }}</td>
						<td>{{ data_get($value, 'no_tel') }}</td>
						<td>{{ data_get($value, 'no_faks') }}</td>
						<td>{{ data_get($value, 'email') }}</td>
						<!-- <td>{{ data_get($value, 'maps') }}</td> -->
						<td>
							@if($value->status == '1')
								Aktif
						...
						<td>
							<a href="{!! URL::to('/site/hubungi/edit/'.data_get($value, 'id')) !!}">
								<i class="edit icon"></i>
							</a>
						</td>
					</tr>
				...
				@empty
					<tr>
						<td colspan='9' class="center aligned">Tiada Data</td>
					</tr>
				@endforelse
			</tbody>
		</table>
