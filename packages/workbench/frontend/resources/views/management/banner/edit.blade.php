...
<!-- <style>
	...
	<div class="ui attached segment">
		...
		{!! form()->open()->post()->action(route('site::frontendmanage.postBannerSaveEdit'))->attribute('id', 'formeditbanner')->multipart()->horizontal() !!}
		 <!-- Form::open()->url('/site/banner/saveedit/'.data_get($data, 'id'))->method('POST')->class('ui form horizontal')->multipart(); !!} -->
		<input type="hidden" name="banner_id" id="banner_id" value="{{ data_get($data, 'id') }}" >
			...
			<div class="field" id="seldesc">
				...
				<textarea type="textarea"
						  ...
						  id="iddesc"
						  onkeyup = "this.value = this.value.toUpperCase();">{{ data_get($data, 'keterangan') }}</textarea>
			</div>
			...
			<div class="field">
				...
				@else
					<a target="_blank" href="{!! URL::to(data_get($data, 'path')) !!}"><img src="{!! URL::to(data_get($data, 'path')) !!}" alt="{{ data_get($data, 'filename') }}" style="width: 500px"></a>
				@endif
			</div>
