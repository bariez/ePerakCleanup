...
<!-- <style>
	...
	<div id="actionbar" class="ui two column grid content__body p-x-3 p-y-1 m-b-0" >
		...
		<div class="column right aligned middle aligned">
			<a class="ui button green" href="{!! URL::to('site/notis/add') !!}" id="addbutton">
	...
	<div class="ui attached segment">
		<table id="listnotis" class="ui celled table" style="width:100%">
			<thead>
				<tr>
					...
					<th style="text-align: center;">Tarikh Notis</th>
					...
					<tr  style="text-align: center;">
						...
						<td>
							<a target="_blank" href="{!! URL::to(data_get($value, 'path')) !!}">
								<img src="{!! URL::to(data_get($value, 'path')) !!}" alt="{{ data_get($value, 'filename') }}" style="width: 300px">
							</a>
						</td>
						<td>{{ data_get($value, 'tajuk') }}</td>
						<td>{{ data_get($value, 'ringkasan') }}</td>
						<td>{{ date("d-M-Y", strtotime(data_get($value, 'tarikh_notis'))) }}</td>
						...
						<td>
							<a href="{!! URL::to('/site/notis/edit/'.data_get($value, 'id')) !!}">
								<i class="edit icon"></i>
							</a>
						</td>
					</tr>
