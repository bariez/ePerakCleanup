...
<div id="actionbar" class="ui two column grid content__body p-x-3 p-y-1 m-b-0" >
 ...
 <a class="ui button" href="{!! URL::to('dataentry/searchkampung/index') !!}" id="backbutton"><i class="material-icons left"></i><span>Kembali</span></a>
 @else

<a class="ui button" href="{!! URL::to('dataentry/searchkampung/mainmenu/'.$idkampung.'/1/1/0') !!}" id="backbutton"><i class="material-icons left"></i><span>Kembali</span></a>
 @endif
                           ...
                           @forelse($ketuaisirumah as $key =>$data)
                             <tr  style="text-align: center;">
                                 ...
                                 <td>{{data_get($data,'Nama')}}</td>
                                  <td><a href="{!! URL::to('/dataentry/searchkampung/isirumah/viewketua/'.data_get($data,'id').'/'.$idkampung) !!}" data-tooltip="Paparan" data-position="bottom center"><i class="eye icon"></i></a>
                                     @if(data_get($roleuser,'role_id')==2 || data_get($roleuser,'role_id')==4)
                                     <a href="{!! URL::to('/dataentry/searchkampung/isirumah/ahliisirumah/'.$idkampung.'/'.data_get($data,'rumah.id')) !!}" data-tooltip="Ahli Isi Rumah" data-position="bottom center"><i class="user plus icon"></i></a>
                                     @else
                                      <a href="{!! URL::to('/dataentry/searchkampung/isirumah/editketua/'.data_get($data,'id').'/'.$idkampung) !!}" data-tooltip="Kemaskini" data-position="bottom center"><i class="edit icon"></i></a>
                                      <a href="{!! URL::to('/dataentry/searchkampung/isirumah/ahliisirumah/'.$idkampung.'/'.data_get($data,'rumah.id')) !!}" data-tooltip="Tambah Ahli Isi Rumah" data-position="bottom center"><i class="user plus icon"></i></a>
                                      @endif
                                       @if(data_get($roleuser,'role_id')!=2 && data_get($roleuser,'role_id')!=4)
                                      <a onclick="return confirm('Adakah anda pasti untuk hapus?');" href="{!! URL::to('dataentry/searchkampung/isirumah/deleteketua/'.data_get($data,'id').'/'.$idkampung.'/'.data_get($data,'rumah.id')) !!}" data-tooltip="Padam" data-position="bottom center"><i class="trash alternate icon" style="color:red"></i></a>
                                      @endif
          ...
          Import Data
           ...
           {!! form()->open()->post()->action(route('dataentry::searchkampung.importketuarumah'))->attribute('id', 'formkemudahan')->multipart()->horizontal() !!}
            ...
            <div class="field">
              ...
              <div class="field">
                 <a target="_blank" href="{{ URL::asset('importKetuaRumah2.xlsx') }}"><i class="file alternate outline icon"></i><span>Contoh Fail</span>
                </a>
                </div>
            <div class="field">
