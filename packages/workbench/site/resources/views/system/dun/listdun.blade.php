...
<div id="actionbar" class="ui two column grid  p-x-3 p-y-1 m-b-0" >
    ...
    <div class="column right aligned middle aligned">
           ...
           <a class="ui button green" href="{!! URL::to('site/dun/adddun') !!}" id="addbutton"><i class="icon plus"></i><span>Tambah</span></a>
            	...
            	<table id="listparlimen" class="ui celled table" style="width:100%">
                        <thead>
                            <tr>
                                ...
                                <th style="text-align: center;">Nama Dun</th>
                                <th style="text-align: center;">Nama Ahli Dun</th>
                         ...
                         <tbody>
                           <?php $i=1; ?>
                           @forelse($dun as $key =>$data)
                             <tr  style="text-align: center;">
                                 <td>{{$i}}</td>
                                 <td>{{data_get($data,'parlimen.NamaParlimen')}}</td>
                                 <td>{{data_get($data,'NamaDun')}}</td>
                                 <td>{{data_get($data,'AhliDun')}}</td>
                                  <td><a href="{!! URL::to('/site/dun/viewdun/'.data_get($data,'id')) !!}"><i class="eye icon"></i>
                                  </a>
                                      <a href="{!! URL::to('/site/dun/editdun/'.data_get($data,'id')) !!}"><i class="edit icon"></i></a>
