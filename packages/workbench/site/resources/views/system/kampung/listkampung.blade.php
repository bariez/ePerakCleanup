...
<div id="actionbar" class="ui two column grid  p-x-3 p-y-1 m-b-0" >
    ...
    <div class="column right aligned middle aligned">
           ...
           <a class="ui button green" href="{!! URL::to('site/kampung/addkampung') !!}" id="addbutton"><i class="icon plus"></i><span>Tambah</span></a>
...
<div class="ui attached segment raised">
            	...
            	<table id="listparlimen" class="ui celled table" style="width:100%">
                        <thead>
                         ...
                         <tbody>
                           <?php $i=1; ?>
                           @forelse($kampung as $key =>$data)
                             <tr  style="text-align: center;">
                                 <td>{{$i}}</td>
                                 <td>{{data_get($data,'parlimen.NamaParlimen')}}</td>
                                 <td>{{data_get($data,'dun.NamaDun')}}</td>
                                 <td>{{data_get($data,'daerah.NamaDaerah')}}</td>
                                 <td>{{data_get($data,'mukim.NamaMukim')}}</td>
                                 <td>{{data_get($data,'NamaKampung')}}</td>
                                  <td><a href="{!! URL::to('/site/kampung/viewkampung/'.data_get($data,'id')) !!}"><i class="eye icon"></i>
                                  </a>
                                      <a href="{!! URL::to('/site/kampung/editkampung/'.data_get($data,'id')) !!}"><i class="edit icon"></i></a>
